---
name: manifest
description: >-
  Reverse-engineers active project code into DESIGN.md and PRODUCT.md architectural specifications. Triggers Floor 42 command /manifest.
---

# /manifest — PixelCrew Command

Inspect codebase AST and generate comprehensive architectural documentation:
1. Reverse-engineer design system tokens, color palettes, and typography into DESIGN.md.
2. Compile product vision, user personas, architecture stack, and feature roadmap into PRODUCT.md.
3. Format output with clear markdown headings and executable specifications.
