---
name: chromatic
description: >-
  Injects curated HSL color tokens, dark mode elevation surfaces, atmospheric accent tiers. Triggers Floor 42 command /chromatic.
---

# /chromatic — PixelCrew Command

Apply PixelCrew Chromatic color strategy for: $ARGUMENTS.
1. Define coherent HSL color tokens for primary, accent, and surface-0 through surface-3.
2. Eliminate generic purple/cyan neon glow on flat black.
3. Guarantee WCAG AA/AAA contrast compliance across all light and dark mode surfaces.
